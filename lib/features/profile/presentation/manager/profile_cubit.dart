...
class ProfileCubit extends Cubit<ProfileState> {
  ...
  void getDataUserInfoProfile() async {
    try {
      emit(state.copyWith(
          isLoadingDataUserProfile: true, errorFetchDataUserInfoProfile: null));
      var result = await repo.getDataUserProfileRepo();
      result.fold(
        (error) {
          log('❌ Error fetching user profile: ${error.message}');
          emit(state.copyWith(
            errorFetchDataUserInfoProfile: error,
            isLoadingDataUserProfile: false,
          ));
        },
        (dataResponse) {
          log('✅ Successfully fetched user profile data');
          emit(state.copyWith(
            dataUserInfoProfile: dataResponse,
            isLoadingDataUserProfile: false,
            errorFetchDataUserInfoProfile: null,
          ));
        },
      );
    } catch (e) {
      log('❌ Exception in getDataUserInfoProfile: $e');
      emit(state.copyWith(
        errorFetchDataUserInfoProfile: Failure(message: e.toString()),
        isLoadingDataUserProfile: false,
      ));
    }
  }
  ...
  Future<void> getDataSavedCourse() async {
    ...
    var result =
        await repo.getDataSavedCoursesRepo(state.dataSavedcourses.currentPage);
